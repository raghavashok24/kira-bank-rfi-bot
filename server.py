...
LOCAL-FIRST: this app is designed to run on a normal machine with a
persistent filesystem, NOT on an ephemeral-disk PaaS. The SQLite database
next to this file (bank_rfi_bot.db) is the durable store: it survives
restarts, so the one-time full-history pretraining happens exactly once,
and every restart after that resumes instantly from the already-trained,
persisted model instead of re-crawling Sumsub and retraining from scratch.
(The old Render free-tier deployment wiped the DB on every restart, which
is exactly what made the bot appear to "restart training every time".)
...
LIFECYCLE:
  1. PRETRAIN (first run only): if the database is empty at startup, the bot
     automatically runs an EXHAUSTIVE full-history backfill (every calendar
     month back to FULL_HISTORY_MONTHS, no early-stop) so every single past
     transaction in company history is ingested, then trains the model once
     on all of it and persists both the data and the trained model.
  2. NEW TRANSACTIONS are ADDED TO THE DATABASE as they happen -- via the
     Sumsub webhook (if registered; needs a publicly reachable URL, e.g. an
     ngrok/cloudflared tunnel when running locally) and via the polling
     sweep every INGEST_INTERVAL_MINUTES that re-checks the last couple of
     days (the reliable mechanism when no webhook can reach this machine).
     Each new incoming transaction is scored with the CURRENT model; if it
     scores at/above SLACK_HIGH_RISK_THRESHOLD it triggers the one and only
     kind of Slack notification this bot sends.
  3. RETRAINING is no longer triggered by every event. The model retrains
     only when its actual training signal changes -- i.e. when the set of
     labeled examples changes (a transaction newly gains the bank-rfi tag,
     or a previously-open review concludes). See retrain_if_labels_changed.
...
INGEST_INTERVAL_MINUTES = int(os.environ.get("INGEST_INTERVAL_MINUTES", "60"))  # polling sweep for NEW transactions (the primary ingest path when no webhook can reach this machine)
# How far back the one-time pretraining backfill walks (in calendar months)
# when the database is empty on startup. 120 months = 10 years -- meant to
# comfortably cover the company's entire transaction history. The pretrain
# crawl is EXHAUSTIVE (no empty-streak early stop), so every month in this
# range is actually queried.
FULL_HISTORY_MONTHS = int(os.environ.get("FULL_HISTORY_MONTHS", "120"))
# Only newly-ingested transactions whose own transaction date is within this
# many days are eligible for an at-risk Slack alert -- so a backfill/import
# that happens to pull in older rows can never spam the channel about
# transactions that aren't actually "incoming".
RECENT_ALERT_WINDOW_DAYS = int(os.environ.get("RECENT_ALERT_WINDOW_DAYS", "7"))
...
# Risk score (0-1) at or above which a NEWLY-INGESTED incoming transaction
# triggers a Slack alert -- see maybe_alert_at_risk_transaction. This is the
# ONLY Slack notification the bot sends: no startup messages, no "new tag
# confirmed" messages, no batch "scores changed after retrain" messages --
# exactly one message per new incoming transaction the model flags at risk.
...
def load_model_state():
    ...
    path (see _maybe_pretrain_on_first_run) rebuilds everything from
...
def log_predictions(scored: list[dict]) -> list[dict]:
    ...
    downstream consumers) to react only to real changes, not
...
def maybe_alert_at_risk_transaction(row: dict) -> None:
    """THE one and only Slack notification: fires when a transaction that is
    NEW TO THE DATABASE (just ingested via the webhook or the polling sweep)
    is scored by the current model at/above SLACK_HIGH_RISK_THRESHOLD.
    Everything else that used to post to Slack (startup status, "new bank-rfi
    tag confirmed", batched "scores changed after retrain") was removed on
    purpose -- the channel should only ever hear about an incoming
    transaction the model flags at risk, nothing else.

    Guards, in order:
      - Slack not configured -> silent no-op.
      - Transaction already carries the bank-rfi tag -> skip (that's ground
        truth, not a prediction -- and alerting on it would just restate
        what Sumsub already shows).
      - Transaction date older than RECENT_ALERT_WINDOW_DAYS -> skip, so a
        backfill/import pulling in historical rows can never flood the
        channel with "alerts" about months-old transactions.
      - The initial full-history pretraining backfill is still running ->
        skip; the model isn't fully trained yet and half-trained scores
        shouldn't page anyone.
    Never raises -- an alerting problem must never break ingestion."""
    try:
        if not SLACK_BOT_TOKEN or not SLACK_CHANNEL:
            return
        if row.get("is_bank_rfi"):
            return
        if _backfill_history_job.get("running"):
            return
        dt = _parse_dt(row)
        now_naive = datetime.now(timezone.utc).replace(tzinfo=None)
        if dt is None or (now_naive - dt).days > RECENT_ALERT_WINDOW_DAYS:
            return
        _ensure_bundle_loaded()
        if _current_bundle is None:
            return
        txns = all_transactions()
        feat = extract_features(row, build_applicant_history_index(txns))
        scored = score_transaction(feat, _current_bundle)
        if scored["risk_score"] < SLACK_HIGH_RISK_THRESHOLD:
            return
        reasons = scored.get("reasons") or ["no single dominant factor -- combination of weaker signals"]
        reason_lines = "\n".join(f"    - {r}" for r in reasons[:3])
        text = (
            f":rotating_light: *Incoming transaction flagged at risk for \"bank rfi\"* "
            f"({scored['risk_score']*100:.1f}% ≥ {SLACK_HIGH_RISK_THRESHOLD*100:.0f}% threshold)\n"
    ...
    except Exception:  # noqa: BLE001 -- alerting must never break the ingest path
        logger.exception("At-risk Slack alert failed (non-fatal)")
...
# Broadened beyond the original ("bank_rfi", "is_rfi", "_tag") to also catch
# any feature named after Sumsub tags generally, reviewer notes, or review
# status -- none of which exist at prediction time for a brand-new incoming
# transaction, so none may ever be a model input.
_FORBIDDEN_FEATURE_MARKERS = ("bank_rfi", "is_rfi", "tag", "note", "review")
...
def analyze_patterns(bank_rfi_features: list[dict], other_features: list[dict], n_excluded_open: int = 0) -> dict:
    ...
    for feat in NUMERIC_FEATURES:
        ...
        # A feature that's constant (or near-constant) across both groups makes
        # mannwhitneyu's normal approximation divide by zero and return p=nan
        # rather than raising -- and one single NaN anywhere in this dict makes
        # the whole /api/patterns JSON response unserializable (HTTP 500),
        # which in turn blanks every panel on the dashboard. Such a feature
        # carries no signal anyway, so skip it entirely.
        if not all(math.isfinite(v) for v in (p_val, rfi_mean, other_mean)):
            continue
        finding = {
...
# then shallow gradient-boosted trees (see _make_hgb) with a logistic
# regression alongside purely for explainability. Never a black box: every
# score comes with matched reasons and the nearest past bank-rfi transactions.
# ============================================================================
...
# Candidate inverse-regularization strengths for the logistic regression.
# With only a few dozen positives, the right amount of L2 shrinkage is the
# single most important knob against memorization -- so instead of
# hard-coding one value, the best C is picked per-retrain by stratified-CV
# PR-AUC (the metric that actually matters for a rare-event detector; see
# _cross_val_metric's docstring). The grid is deliberately small and skewed
# toward strong regularization.
...
def _make_hgb() -> HistGradientBoostingClassifier:
    """The PRIMARY risk scorer: shallow gradient-boosted trees. Chosen over
    logistic regression on measured out-of-fold results against this
    account's real data (6,895 labeled, 43 positives): OOF PR-AUC ~0.057 vs
    ~0.021 for the best logistic regression, and 6x the catches in the
    top-1%-of-scores alert band. The reason it wins here is interactions: a
    bank RFI isn't signaled by "large amount" OR "outbound" OR "thin
    applicant history" independently (which is all a linear model can
    express) but by their CONJUNCTION, which depth-3 trees represent
    directly. Depth is capped at 3 and the learning rate kept low
    specifically because 43 positives can't support anything deeper without
    memorizing them."""
    return HistGradientBoostingClassifier(
        max_depth=3, max_iter=200, learning_rate=0.05, class_weight="balanced", random_state=0,
    )


def _select_best_C(X: np.ndarray, y: np.ndarray, cv_folds: int) -> tuple[float, dict]:
    """Picks the C from C_GRID with the best cross-validated PR-AUC
    (stratified folds, fresh classifier per fold). Falls back to 0.3 -- the
    previous hard-coded value -- when there's not enough data to fold. Also
    returns the per-C scores so the model-version metrics can show WHY this
    C won, not just which one did."""
    scores: dict = {}
    if cv_folds < 2:
        return 0.3, scores
    best_c, best_score = 0.3, -1.0
    for c in C_GRID:
        s = _cross_val_metric(lambda c=c: _make_lr(c), X, y, cv_folds, scoring="average_precision")
        scores[str(c)] = s
...
def _temporal_holdout_metrics(labeled_feats: list[dict], vocab: dict) -> dict:
    ...
    try:
        ...
        # Evaluates the SAME architecture actually being deployed (see
        # _make_hgb) -- a holdout score for a different model family than
        # the one serving predictions would answer the wrong question.
        holdout_clf = _make_hgb()
...
def train_model(transactions: list[dict]) -> ModelBundle:
    """SCORE SEMANTICS (this fixed a real bug -- don't reintroduce
    calibration without re-reading this): the risk score served by this
    model is a RELATIVE risk score from a class-balanced model, NOT the
    real-world probability that a bank RFI occurs. An earlier version
    wrapped the classifier in CalibratedClassifierCV, which maps scores to
    true real-world frequencies -- statistically defensible, but with a
    ~0.6% base rate (43 RFIs in ~6,900 transactions) it squashed EVERY
    score to the 0-2% range: even the 43 CONFIRMED bank-rfi transactions
    scored at most 1.7%, transactions nearly identical to past RFI cases
    looked "not picked up", and the 75% Slack alert threshold could
    mathematically never fire. Under the balanced scorer used now, a
    transaction matching the historical RFI profile scores high (confirmed
    RFI cases have a median score of ~0.9 when re-scored) and a typical
    safe transaction scores near 0 -- which is what a threshold-based
    alerting system and a human reading the dashboard both actually need.
    The tradeoff, stated honestly: the score is a ranking/matching signal,
    not a frequency ("0.9" means "strongly matches the RFI profile", not
    "90% of such transactions get an RFI").

    Against memorization on a few dozen positives: MAX_CATEGORICAL_VOCAB
    bounds how many one-hot dimensions a single categorical feature can add
    (see _vocab_for), the boosted trees are capped shallow (see _make_hgb),
    and the explainability regression picks its regularization by
    cross-validated PR-AUC (see _select_best_C).
    ...
    isn't drowned out, StratifiedKFold keeps the true bank-rfi ratio
    intact in every CV fold instead of risking a fold with zero
    ...
    if len(bank_rfi_feats) < MIN_ML_SAMPLES or len(confirmed_other_feats) < MIN_ML_SAMPLES:
            ...
            metrics={
                **base_metrics,
                "reason": (
                    ...
                    f"switching to the ML classifier -- currently {len(bank_rfi_feats)} bank-rfi / "
        ...
        # The regularization search only affects the EXPLAINABILITY
        # regression (whose coefficients feed the feature-importance
        # panel) -- the primary scorer's capacity is fixed in _make_hgb.
        best_C, c_grid_scores = _select_best_C(X_labeled, y, cv_folds)
        explain_clf = _make_lr(best_C)
        explain_clf.fit(X_labeled, y)

        # Primary scorer: shallow gradient-boosted trees, fit on 100% of
        # the labeled data. Deliberately NOT wrapped in calibration -- see
        # this function's docstring for why that broke alerting outright.
        clf = _make_hgb()
        clf.fit(X_labeled, y)

        cv_auc = _cross_val_metric(_make_hgb, X_labeled, y, cv_folds, scoring="roc_auc")
        cv_pr_auc = _cross_val_metric(_make_hgb, X_labeled, y, cv_folds, scoring="average_precision")
        ...
        bundle = ModelBundle(
            ...
            metrics={
                **base_metrics,
                "cv_folds": cv_folds,
                "model_family": "HistGradientBoostingClassifier (shallow, class-balanced)",
                "score_scale_note": (
                    "Risk scores are RELATIVE profile-match scores from a class-balanced model, not "
                    "real-world probabilities -- a confirmed-RFI-like transaction scores near 1, a typical "
                    "safe one near 0. Calibrated frequencies were deliberately removed: at a ~0.6% base "
                    "rate they compressed every score into 0-2%, which made thresholds meaningless."
                ),
                "selected_C_for_explainability_regression": best_C,
                "c_grid_pr_auc": c_grid_scores,
                "cv_roc_auc": cv_auc,
                "cv_roc_auc_note": (
...
def ingest_single_txn(client: SumsubClient, txn_id: str) -> dict | None:
    ...
    # Whether this transaction is being seen for the FIRST time (vs. a
    # re-fetch/update of a row already stored) -- this is what decides
    # whether it counts as an "incoming transaction" eligible for an
    # at-risk Slack alert (see maybe_alert_at_risk_transaction). Checked
    # BEFORE the upsert, transient (underscore) so it's never persisted.
    row["_new_to_db"] = get_transaction(row["txn_id"]) is None
...
def _query_txn_window(client: SumsubClient, window_start: datetime, window_end: datetime, limit: int,
                       seen_ids: set, max_pages: int = 500, alert_on_new: bool = False) -> dict:
    ...
    while pages < max_pages:
        ...
        for item in items:
            ...
            if row:
                scanned += 1
                new_rfi += 1 if row["is_bank_rfi"] else 0
                # The polling sweep is how NEW incoming transactions reach
                # a locally-run bot with no publicly-reachable webhook URL
                # -- so the at-risk check has to happen here too, not just
                # in the webhook path. Only for rows genuinely new to the
                # DB, and never during the deep historical crawl
                # (alert_on_new=False there).
                if alert_on_new and row.get("_new_to_db"):
                    maybe_alert_at_risk_transaction(row)
        if len(items) < limit:
            break
        if new_this_page == 0:
...
def backfill_from_txn_query(client: SumsubClient, days_back: int = 2, limit: int = 100) -> dict:
    ...
    seen_ids = set()
    w = _query_txn_window(client, now - timedelta(days=days_back), now, limit, seen_ids, alert_on_new=True)
...
# Diagnostics for _maybe_pretrain_on_first_run (defined further down, after
...
def process_webhook_event(client: SumsubClient, event: dict) -> tuple[dict | None, str]:
    ...
    # transaction" -- so label-change handling fires once
...
def _labeled_fingerprint(transactions: list[dict]) -> str:
    """Order-independent hash of the model's actual training signal: the set
    of labeled examples (every bank-rfi-tagged transaction plus every
    transaction whose review has concluded) and their labels. If two
    database states produce the same fingerprint, training on them produces
    the same labeled dataset -- so there is nothing new for the model to
    learn and retraining is pure waste. This is what lets ordinary
    transaction ingestion just ADD to the database without kicking off a
    retrain (see retrain_if_labels_changed): only a label change (new tag,
    or a review concluding) moves this hash."""
    parts = sorted(
        f"{t.get('txn_id')}:{1 if t.get('is_bank_rfi') else 0}"
        for t in transactions
        if t.get("is_bank_rfi") or t.get("review_status") not in OPEN_STATUSES
    )
    return hashlib.sha256("|".join(parts).encode()).hexdigest()


def retrain_if_labels_changed(reason: str = "") -> bool:
    """Retrains ONLY if the labeled training set actually changed since the
    current model was trained -- the replacement for the old behavior of
    running a full retrain on every single webhook event and every hourly
    sweep (which is what made the bot look like it was 'restarting training
    every time'). Cheap when nothing changed: one DB read + one hash, no
    model fitting. Returns True iff a retrain actually ran."""
    global _current_bundle
    _ensure_bundle_loaded()
    txns = all_transactions()
    if not txns:
        return False
    fp = _labeled_fingerprint(txns)
    current_fp = getattr(_current_bundle, "labeled_fingerprint", None) if _current_bundle else None
    if _current_bundle is not None and current_fp == fp:
        return False
    logger.info("Labeled training set changed%s -- retraining.", f" ({reason})" if reason else "")
    retrain_only()
    return True
...
def retrain_only():
    ...
    calls. Called from retrain_if_labels_changed (the normal path), from the
    initial pretraining backfill, and from the manual import flow -- NOT on
    every webhook event anymore; ordinary ingestion only adds rows to the
    database and lets the label-change check decide."""
    global _current_bundle
    try:
        txns = all_transactions()
        if txns:
            _current_bundle = train_model(txns)
            # Stamp the bundle with the labeled-set fingerprint it was
            # trained on, so retrain_if_labels_changed can tell "nothing new
            # to learn" apart from "labels changed" without retraining.
            _current_bundle.labeled_fingerprint = _labeled_fingerprint(txns)
            ...
            try:
                ...
                # Audit trail only -- deliberately NO Slack notification
                # here. Retrains shift many scores at once and batching
                # those into channel messages was exactly the spam the
                # at-risk-only alert policy removed; the sole notification
                # now lives in maybe_alert_at_risk_transaction.
...
def run_ingest_and_retrain():
    """The POLLING SWEEP (every INGEST_INTERVAL_MINUTES): pulls the last
    couple of days of transactions from Sumsub into the database -- for a
    locally-run bot with no publicly reachable webhook URL, this is the
    PRIMARY way new transactions get added. Each transaction new to the DB
    is scored on the spot and Slack-alerted if at risk (inside
    _query_txn_window via alert_on_new). Afterwards the model retrains ONLY
    if the labeled training set actually changed (new bank-rfi tag, or a
    review concluded) -- never unconditionally on a timer."""
    logger.info("Starting polling ingest sweep")
    client = get_client()
    if client is None:
        logger.warning("Credentials not set yet -- skipping ingestion sweep.")
    else:
        try:
            result = run_full_backfill(client)
            logger.info("Ingestion result: %s", {k: v for k, v in result.items() if k != "results"})
        except Exception:
            logger.exception("Ingestion sweep failed; checking for label changes on existing data anyway")
    retrain_if_labels_changed(reason="polling sweep")
...
def _maybe_pretrain_on_first_run():
    """The one-time PRETRAINING step: on the FIRST run against an empty
    database (which, locally, with a persistent filesystem, genuinely means
    "never ingested anything yet" -- not "the platform wiped the disk
    again"), automatically crawl EVERY past transaction in company history
    (an exhaustive month-by-month walk over FULL_HISTORY_MONTHS with the
    empty-streak early-stop disabled, so no month is skipped) and then
    train the model once on all of it. Both the data and the trained model
    persist in SQLite, so this never runs again on subsequent restarts --
    the bot resumes instantly from the saved model instead of retraining.
    Runs on a plain background thread (not BackgroundTasks, since there's
    no request/response cycle at startup to hang it off of) so it doesn't
    block the app from starting to serve traffic. Uses the SAME job/state
    (_backfill_history_job) as the manual "Run full historical backfill"
    button, so its progress is visible in exactly the same place -- and
    at-risk Slack alerts are suppressed while it runs (see
    maybe_alert_at_risk_transaction), so pretraining can never flood the
    channel."""
    ...
    try:
        ...
        logger.exception("Could not check transaction count for first-run pretraining")
        return
    if existing != 0:
        _startup_auto_backfill.update(
            attempted=False,
            reason=f"{existing} transaction(s) already stored -- pretraining already happened, resuming from the saved model.",
            months_back=None,
        )
        return
    client = get_client()
    if client is None:
        ...
        return
    months_back = FULL_HISTORY_MONTHS
    _backfill_history_job.update(running=True, months_done=0, months_total=months_back, result=None,
                                  started_at=datetime.now(timezone.utc).isoformat(), finished_at=None)
    _startup_auto_backfill.update(
        attempted=True,
        reason="First run against an empty database -- pretraining now: exhaustively ingesting every past "
               "transaction in company history, then training the model once on all of it.",
        months_back=months_back,
    )
    logger.info("First run: pretraining -- exhaustive %d-month full-history backfill, then a one-time train.", months_back)
    threading.Thread(target=_run_backfill_history_job_in_background, args=(client, months_back, True), daemon=True).start()
...
def on_startup():
    ...
    try:
        ...
        logger.info(
            "Polling ingest sweep scheduled every %d minute(s) -- new transactions are added to the "
            "database and scored; the model itself retrains only when the labeled training set changes.",
            INGEST_INTERVAL_MINUTES,
        )
    except Exception:  # noqa: BLE001
        logger.exception(
            "Failed to start the background scheduler -- webhook-driven ingestion still works; "
            "the polling sweep will not run until the next restart."
        )
    try:
        _maybe_pretrain_on_first_run()
    except Exception:  # noqa: BLE001
        logger.exception(
            "First-run pretraining check failed at startup -- not fatal, use the dashboard's "
            "manual \"Run full historical backfill\" button instead."
        )


def _db_likely_ephemeral() -> bool:
    """Warns only when the process is actually running on a PaaS that wipes
    the filesystem on restart (Render sets the RENDER env var; a generic
    dyno-style platform sets DYNO). Locally -- the supported way to run this
    bot -- the default ./bank_rfi_bot.db path is on a normal persistent disk
    and is exactly where the data and pretrained model are supposed to
    live, so no warning."""
    try:
...
def _process_webhook_event_in_background(event: dict, event_row_id: int):
    ...
    try:
        ...
        if row:
            # The webhook's job is to ADD/UPDATE the transaction in the
            # database -- NOT to retrain on every event. Retraining happens
            # only when the labeled training set actually changed (a new
            # bank-rfi tag, a review concluding, a labeled row deleted);
            # a plain "new transaction came in" just grows the database and
            # gets scored against the existing pretrained model.
            if row.get("deleted"):
                ...
                tag_note = "transaction added/updated"
            logger.info("Webhook event processed (%s): %s", tag_note, row["txn_id"])
            # The ONLY Slack notification: a transaction new to the DB whose
            # score crosses the at-risk threshold (all guards live inside).
            if row.get("_new_to_db"):
                maybe_alert_at_risk_transaction(row)
            retrain_if_labels_changed(reason=f"webhook: {tag_note}")
        else:
...
function renderSetup(s) {
  ...
  if (s.slack_configured) {
    rows.push({icon: "good", text: `<strong>Slack notifications are on.</strong> A message is posted ONLY when a newly-ingested incoming transaction is scored at/above ${(s.slack_high_risk_threshold*100).toFixed(0)}% predicted risk (set via SLACK_HIGH_RISK_THRESHOLD). No startup messages, no per-retrain digests, no "tag confirmed" messages.`});
  }
  ...
  if (s.total_transactions === 0) {
    ...
    rows.push({icon: "good", text: `<strong>${s.total_transactions.toLocaleString()} transaction(s) ingested</strong>, ${s.total_bank_rfi.toLocaleString()} tagged "${s.bank_rfi_tag}" so far -- all pulled from the Sumsub API. New transactions are added to the database via the webhook and via a polling sweep every ${s.ingest_interval_minutes} minute(s); the model itself retrains only when the labeled training set actually changes (a new tag, or a review concluding), never just because a transaction arrived.`});
  }
...
if __name__ == "__main__":
    # Local entrypoint: `python server.py` -- equivalent to
    # `uvicorn server:app --host 0.0.0.0 --port 8000` (set PORT to change).
    import uvicorn
