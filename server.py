...
def _temporal_holdout_metrics(labeled_feats: list[dict], vocab: dict, C: float = 0.3) -> dict:
...
def train_model(transactions: list[dict]) -> ModelBundle:
    ...
    if len(bank_rfi_feats) < MIN_ML_SAMPLES or len(confirmed_other_feats) < MIN_ML_SAMPLES:
        ...
        # Pick the regularization strength that actually generalizes best
        # (by stratified-CV PR-AUC) instead of trusting one hard-coded C.
        best_C, c_grid_scores = _select_best_C(X_labeled, y, cv_folds)

        explain_clf = _make_lr(best_C)
        explain_clf.fit(X_labeled, y)

        clf = CalibratedClassifierCV(_make_lr(best_C), method="sigmoid", cv=cv_folds)
        clf.fit(X_labeled, y)

        clf_factory = lambda: _make_lr(best_C)  # noqa: E731
        ...
        temporal_metrics = _temporal_holdout_metrics(labeled_feats, vocab, C=best_C)
